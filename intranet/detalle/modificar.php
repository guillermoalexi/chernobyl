...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM detalleventa WHERE idDetalleventa = $id";
    $rotulo = "MODIFICAR";
    bd_conectar();
    ...
    <header>
        ...
        <form  name="InsUpd" method="POST" action="editar_grabar.php">
            <fieldset style="width: 70%; margin-left: 150px; padding: 20px;">
                ...
                <legend ><?= $rotulo ?> TRANSACCION </legend>
                <input type="text" name="txtId"  value="<?= $proveedor['idDetalleventa'] ?>"
                       hidden="hidden"/>
                CODIGO DE PRODUCTO<br/>
                <input type="text" name="txtNom" value="<?= $proveedor['articulo'] ?>"
                       placeholder="Ingrese el codigo de producto" required="required"/><br/>
                <br/>CODIGO DE VENTA<br/>
                <input type="text" name="txtSim" value="<?= $proveedor['factura'] ?>"
                       placeholder="Ingrese codigo de venta" required="required"/><br/>            
                <br/>
                <br/>CANTIDAD <br/>
                <input type="text" name="txtTelf" value="<?= $proveedor['cantidad'] ?>"
                       placeholder="Ingrese cantidad de productos" required="required"/><br/>
                <br/><br/>
